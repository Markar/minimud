...
class OrcSlaver(XYGridRoom):
    def at_object_creation(self):
        super().at_object_creation()
        SpawnMob(self, 8100, 11, 1, "ORC_SLAVER", "crushbone")


class OrcOracle(XYGridRoom):
    def at_object_creation(self):
        super().at_object_creation()
        SpawnMob(self, 9800, 20, 1, "ORC_ORACLE", "crushbone")


class OrcEmissary(XYGridRoom):
    def at_object_creation(self):
        super().at_object_creation()
        SpawnMob(self, 12150, 25, 2, "ORC_EMISSARY", "crushbone")


class OrcRoyalGuard(XYGridRoom):
    def at_object_creation(self):
        super().at_object_creation()
        SpawnMob(self, 13820, 30, 2, "ORC_ROYAL_GUARD", "crushbone")


class OrcTaskmaster(XYGridRoom):
    def at_object_creation(self):
        super().at_object_creation()
        SpawnMob(self, 9200, 10, 2, "ORC_TASKMASTER", "crushbone")


class OrcWarlord(XYGridRoom):
    def at_object_creation(self):
        super().at_object_creation()
        SpawnMob(self, 15200, 20, 2, "ORC_WARLORD", "crushbone")


class OrcLord(XYGridRoom):
    def at_object_creation(self):
        super().at_object_creation()
        SpawnMob(self, 18300, 25, 2, "ORC_LORD", "crushbone")


class OrcEmperor(XYGridRoom):
    def at_object_creation(self):
        super().at_object_creation()
        SpawnMob(self, 20000, 30, 2, "ORC_EMPEROR", "crushbone")
